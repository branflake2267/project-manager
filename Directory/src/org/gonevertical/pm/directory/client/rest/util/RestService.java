...
public abstract class RestService<T extends JavaScriptObject> {
  ...
  protected <L extends JsArray<T>> void getList(String url, final RestListHandler<L> handler) {
    url = getUrl(url);
    ...
    builder.setCallback(new RequestCallback() {
      ...
      public void onResponseReceived(Request request, Response response) {
        ...
        if (response.getStatusCode() == 200) {
          L jso = (L) JSONParser.parseLenient(json).isObject().get("items").isObject().getJavaScriptObject().cast();
          handler.onSuccess(jso);
        } else {
          handler.onFailure(new Exception("Exception: getStatusText=" + response.getStatusText()));
        }
